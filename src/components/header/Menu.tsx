  ...
  return (
    <div className={(isOpen ? 'flex' : 'hidden') + ' flex-col gap-2 md:rounded-lg absolute top-0 right-0 md:right-[50px] md:top-[68px] xl:right-[200px] w-[300px] bg-white text-gray-900 z-10 drop-shadow-lg overflow-hidden'}>
      <section className='border-b-2 border-gray-300 flex items-center justify-between h-[68px] px-2'>
        ...
        <div onClick={() => handleClick()} className='hover:scale-110 duration-200 cursor-pointer md:hidden'>
          <IoMdClose fontSize={24} />
        </div>
      </section>
      <section className='flex flex-col gap-2 pl-4 py-2 text-lg border-b-2 border-gray-300 md:border-none'>
        <h3 className='text-lg font-semibold'>Profile</h3>
        <Link href='/profile' className="flex gap-2 items-center cursor-pointer md:hover:font-semibold md:hover:after:content-[''] hover:after:w-2 after:h-6 after:absolute after:right-0 after:bg-gray-900 after:rounded-l-lg">
          <FaUserCircle />
          <p>My profile</p>
        </Link>
      ...
      {userType === 'EMPLOYER'
        ? <section className='flex flex-col gap-2 pl-4 py-2 text-lg md:hidden'>
          ...
          <div className='flex gap-2 items-center cursor-pointer md:hover:font-semibold'>
            <HiDocumentPlus />
